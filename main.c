...
*/
    ...
    if (xcb_connection_has_error(connection)) {
        printf("xcb_connect returned an error on init");
        exit(0);
    }
    ...
    while ((event = xcb_wait_for_event (connection))) {
        switch (event->response_type & ~0x80) {
            ...
            */
                // draw the rectangles 
                xcb_poly_rectangle (connection, window, foreground, 1, rectangle1);
 
                // fill rectangles
                xcb_poly_fill_rectangle ( connection, window, foreground, 1, rectangle1);
